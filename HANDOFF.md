...
## The seed trap, and the day it cost

One test — "arrows are bodies in flight" — failed about one run in
three, and the fix was one line: pin the campaign seed.

A mission's SITE is built from the campaign seed and `newCampaign()`
takes a random one, so every run fought on a different piece of ground.
Once the roadside gained terrain and scattered props, some of those
grounds did not support what the test assumed. This is the trap already
recorded under "Writing tests here", and it caught us anyway.

**What it cost.** Four fixes were attempted before the seed was even
questioned: sweeping for level ground, sweeping bearings with
`Level.hasLOS`, re-seating the target every tick, pinning the archers in
place. Each moved the failure rate around, which reads as progress and
is not — with a random input, no amount of care in the measurement makes
the result stable. One attempt made it NINE runs in ten, because arrows
in flight were chasing a man who teleported out from under them.

**The order to work in** when a test is flaky:

1. Is the input random? Pin the seed FIRST and see whether the result
   goes consistent. Consistently-failing is a solved problem;
   intermittently-failing is not.
2. Only then ask whether the pinned scenario is the one you meant. Here
   the first pinned seed failed 10/10 — the same bug finally holding
   still — and a short seed hunt found several that exercise the
   intended geometry. 4242 is the one pinned.
3. Check against a stash before assuming your own changes caused it.

One more trap inside the trap: a probe reproducing "the same" scenario
is not the same scenario. The seed hunt reported clear line of sight on
every seed tried, including ones the real test failed on, because the
probe skipped the two earlier steps that leave arrows in flight and
bodies moved. Reproduce the whole test or trust none of it.
