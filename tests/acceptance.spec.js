...
test('every piece of kit on the shelf changes something the melee reads', async ({ page }) => {
  await boot(page);
  await newCampaign(page);
  // Two items were still selling gun statistics into a game with no guns.
  // A BANDOLIER multiplied the magazine by 1.5 — wired, genuinely read —
  // but every melee weapon and the bow carry mag: 999, so it bought half
  // again as much of a cap nobody reaches. A STABILISER resisted
  // suppression, which is generated by exactly two code paths: the hitscan
  // gun, and the Titan's LMG. There are no guns.
  //
  // Both were plumbed, which is why nobody noticed: a stat that is read and
  // inert looks identical to one that works. So the check is not "is the key
  ...
  const inert = r.filter((x) => x.moved.length === 0).map((x) => x.id);
  ...
  expect(brace.moved).toContain('guardStr');
  expect(brace.moved).toContain('staggerRes');
});
